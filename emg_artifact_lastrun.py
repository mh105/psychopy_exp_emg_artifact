...
def setupData(expInfo, dataDir=None):
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/alexhe/Dropbox (Personal)/Active_projects/PsychoPy/exp_emg_artifact/emg_artifact_lastrun.py',
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # keep track of which components have finished
    instruct_taskComponents = [text_instruct_task, key_instruct_task]
    for thisComponent in instruct_taskComponents:
        thisComponent.tStart = None
        thisComponent.tStop = None
        thisComponent.tStartRefresh = None
        thisComponent.tStopRefresh = None
    ...
    blocks = data.TrialHandler(nReps=n_blocks + 1, method='sequential', 
    ...
    for thisBlock in blocks:
        ...
        # set up handler to look after randomisation of conditions etc
        trials = data.TrialHandler(nReps=n_videos, method='sequential', 
            extraInfo=expInfo, originPath=-1,
            trialList=[None],
        ...
        for thisTrial in trials:
            ...
            continueRoutine = True
            # update component parameters for each repeat
            thisExp.addData('video.started', globalClock.getTime(format='float'))
            ...
            while continueRoutine and routineTimer.getTime() < 5.0:
                ...
                if movie_video.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > movie_video.tStartRefresh + 5.0-frameTolerance:
                        # keep track of stop time/frame for later
            ...
            for thisComponent in videoComponents:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            thisExp.addData('video.stopped', globalClock.getTime(format='float'))
            # using non-slip timing so subtract the expected duration of this Routine (unless ended on request)
            if routineForceEnded:
                routineTimer.reset()
            else:
            ...
            while continueRoutine and routineTimer.getTime() < 0.2:
                ...
                if sound_beep.status == STARTED:
                    # is it time to stop? (based on global clock, using actual start)
                    if tThisFlipGlobal > sound_beep.tStartRefresh + 0.2-frameTolerance:
                        # keep track of stop time/frame for later
